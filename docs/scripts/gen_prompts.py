...
def main():
    ...
    data = subprocess.run(
        "bun run src/mcp.ts list-prompts --json",
        capture_output=True,
        text=True,
        shell=True,
    ...
    try:
        prompts = json.loads(data.stdout)
    ...
    result = json.loads(subprocess.run(
        "bun run run_handlebars.ts ../templates/prompt.md.hbs".split(),
    ...
    for prompt in result:
        ...
        name = prompt['id']
        doc_path = Path("reference") / Path("prompts") / category / f"{name}.md"
        with mkdocs_gen_files.open(doc_path, mode='w') as fd:
            fd.write(prompt['docContent'])
        ...
        nav_current = nav_map["Reference"]["Prompts"]
