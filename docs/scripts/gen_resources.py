...
def main():
    ...
    data = subprocess.run(
        "bun run src/mcp.ts list-resources --json",
        capture_output=True,
        text=True,
        shell=True,
    ...
    try:
        resources = json.loads(data.stdout)
    except Exception as e:
        print(f"Failed to parse resource list JSON: {e}")
        print(f"STDOUT: {data.stderr}")
        return
    
    
    result = json.loads(subprocess.run(
        "bun run run_handlebars.ts ../templates/resource.md.hbs".split(),
    ...
    for resource in result:
        ...
        name = resource['id']
        doc_path = Path("reference") / Path("resources") / category / f"{name}.md"
        with mkdocs_gen_files.open(doc_path, mode='w') as fd:
            fd.write(resource['docContent'])
        ...
        nav_current = nav_map["Reference"]["Resources"]
